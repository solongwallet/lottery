...
class SolongLottery {
  ...
  static createInitializeInstruction(adminAccountKey, billboardAccountKey, poolAccountKey, programID, fund, price) {
    ...
    dataLayout.encode({
      i: 1,
      // initialize instruct 
      fund: new splToken.u64(fund).toBuffer(),
      price: new splToken.u64(price).toBuffer()
    }, data);
    let keys = [{
  ...
  static async GetLotteryPool(connection, poolAccountKey) {
    ...
    if (resp.result) {
      ...
      for (let i = 0; i < playerCount; i++) {
        ...
        const playerAccountKey = new web3_js.PublicKey(pool.slice(offset, offset + 32)).toBase58();
  ...
  static async GetBillboard(connection, billboardAccountKey) {
    ...
    if (resp.result) {
      ...
      for (let i = 0; i < awardCount; i++) {
        ...
        const key = new web3_js.PublicKey(pool.slice(index, index + 32));
...
// @flow
const poolSpace = 8 + 8 + 8 + 32 + 2 + 1000 * (32 + 2 + 1);
